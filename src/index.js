  ...
  serverFactory: (handler) => {
    return createServer()
      ...
      .on("upgrade", (req, socket, head) => {
        ...
        // Vercel rewrites /wisp/ to the function entrypoint before the
        // upgrade reaches Node, so do not depend on the original URL path.
...
if (process.env.VERCEL !== "1") fastify.listen({ port: PORT, host: HOST }, (err) => {
  if (err) {
    fastify.log.error(err);
    process.exit(1);
  }
